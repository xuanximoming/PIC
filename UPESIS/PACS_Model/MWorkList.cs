...
namespace PACS_Model
{
    ...
    public class MWorkList : ILL.IModel
    {
        ...
        { }
        #region Model
        private string _exam_accession_num;     //检查申请序号
        ...
        private string _patient_local_id;       //检查标识号
        private string _local_id_class;         //检查号类别
        private string _inp_no;                 //住院号
        private string _bed_num;                //床号
        private string _opd_no;                 //门诊号
        private string _birth_place;            //出生地
        private int? _charge_type;              //费用类别
        private string _mailing_address;        //通信地址
        private string _zip_code;               //邮政编号
        private string _telephone_num;          //电话
        ...
        private string _device;                 //检查设备名
        private DateTime? _study_date_time;     //检查日期时间
        private int? _exam_step_status;         //检查完成标志：默认为0,表示还未做检查;1:病人已经到达;2病人规定时间内未到达;3 病人正在被呼叫；
        private int? _report_status;            //报告标志：0.未写，1已写，2正在写
        private int? _is_confirmed;             //收费标志：0.未确认1.已确认收费2.表示退费默认为0
        private int? _match_status;             //匹配标志
        private int? _is_temporary;             //绿色通道标志
        private int? _vip_indicator;            //VIP标志：1-VIP 2-非VIP
        private int? _image_archived;           //图像入库标志：0.未入库1.已入库
        private int? _pre_fetch;                //预取状态标志：0.未执行1.已执行
        private int? _dispatch_status;          //分发标志
        private int? _query_status;             //轮询标志：0，1标志是否轮询
        private DateTime? _query_time;          //轮询时间
        private int? _patient_class;            //入院方式
        private int? _image_counts;             //图像数量
        private DateTime? _scheduled_date;      //预约时间
        private string _sch_operator;           //操作员
        ...
        private string _relevant_lab_test;      //相关化验
        private string _relevant_diag;          //其他诊断
        private string _clin_diag;              //临床诊断
        private string _facility;               //设备名
        private string _out_med_institution;    //外来医疗单位机构
        private decimal? _costs;                //费用
        private decimal? _charges;              //应收费用
        private int? _visit_id;                 //本次住院标志
