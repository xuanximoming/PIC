...
namespace BaseControls
{
    class SkinEngine
    {
        ...
        {
            IntPtr hRgnTemp = ApiWin32.CreateRectRgn(0, 0, bmW, bmH);
            ApiWin32.OffsetRgn(hRgnTemp, xOffset, yOffset);
            ApiWin32.CombineRgn(hRgnClip, hRgnClip, hRgnTemp, ApiWin32.RGN_DIFF);
            ApiWin32.OffsetRgn(hRgnTemp, -xOffset, -yOffset);

            hRgnTemp = ApiWin32.CreateRectRgn(0, 0, bmW, bmH);
            ApiWin32.OffsetRgn(hRgn, rgnX, rgnY);
            ApiWin32.CombineRgn(hRgnTemp, hRgnTemp, hRgn, ApiWin32.RGN_AND);
            ApiWin32.OffsetRgn(hRgn, -rgnX, -rgnY);

            ApiWin32.OffsetRgn(hRgnTemp, xOffset, yOffset);
            ApiWin32.CombineRgn(hRgnClip, hRgnClip, hRgnTemp, ApiWin32.RGN_OR);
            ApiWin32.DeleteObject(hRgnTemp);
        }

        public static Bitmap GetBitmapFromResource(string imgName)
        {
            string path = string.Format("{0}.Properties.Resources", UseNameSpace);
            System.Resources.ResourceManager resourceManager = new System.Resources.ResourceManager(path, typeof(SkinEngine).Assembly);
        ...
        public static void BuildRegion(IntPtr hWnd)
        {
            ...
            ApiWin32.RECT rw = new ApiWin32.RECT();
            ApiWin32.GetWindowRect(hWnd, ref rw);
            ...
            IntPtr hRgn = ApiWin32.CreateRectRgn(0, 0, 0, 0);
            if (hRgn != IntPtr.Zero)
            {
                if (ApiWin32.GetWindowRgn(hWnd, hRgn))
                {
                    hRgnClip = ApiWin32.CreateRectRgn(0, 0, RgnWidth, RgnHeight);
                    if (hRgnClip != IntPtr.Zero)
                    {
                        ...
                        Form.FromHandle(hWnd).BackgroundImage = (Bitmap)SkinEngine.DrawBackGround(RgnWidth, RgnHeight);

                        // Set the new region to the form.
                        ApiWin32.SetWindowRgn(hWnd, hRgnClip, false);

                        // Save the new region size
                        LastRgnWidth = RgnWidth; LastRgnHeight = RgnHeight;
                    }
                }
                ApiWin32.DeleteObject(hRgn);
            }
        }
