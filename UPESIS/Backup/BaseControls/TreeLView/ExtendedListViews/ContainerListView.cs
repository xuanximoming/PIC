...
namespace BaseControls.TreeLView.ExtendedListViews
{
	...
	public class ColumnHeaderCollection: CollectionBase
	{
        ...
        public virtual int Add(BaseControls.TreeLView.ExtendedListViews.ToggleColumnHeader colhead)
		{
        ...
        public virtual void AddRange(BaseControls.TreeLView.ExtendedListViews.ToggleColumnHeader[] items)
		{
			lock(List.SyncRoot)
			{
				for (int i=0; i< items.Length; i++)
	...
	public class ContainerListView: Control
	{
        ...
        protected BaseControls.TreeLView.ExtendedListViews.ColumnHeaderCollection columns;			
        ...
        protected BaseControls.TreeLView.ExtendedListViews.ContainerListViewItem focusedItem;	
		...
		private void Construct()
		{
            ...
            columns = new BaseControls.TreeLView.ExtendedListViews.ColumnHeaderCollection();
        ...
        public BaseControls.TreeLView.ExtendedListViews.ColumnHeaderCollection Columns
		{
			get { return columns; }
		}
		...
		public bool	VisualStyles
		{
			get 
			{
				...
				try
				{
                    val = visualStyles && BaseControls.TreeLView.ExtendedListViews.uxTheme.Wrapper.IsAppThemed();
				}
				catch
				{
					val = visualStyles;
			...
			if (headerStyle != ColumnHeaderStyle.None)
			{
				...
				try
				{
					...
					for (i=0; i<columns.Count; i++)
					{
						colwid = (columns[i].ScaleStyle == ColumnScaleStyle.Spring ? springWid : columns[i].Width);
						if (headerStyle == ColumnHeaderStyle.Clickable && columns[i].Pressed)
                            BaseControls.TreeLView.ExtendedListViews.uxTheme.Wrapper.DrawBackground("HEADER", "HEADERITEM", "PRESSED", hdc,
								lp_scr+last, tp, 
								colwid, headerBuffer,
								lp, tp, r.Width-6, headerBuffer);
						else if (headerStyle != ColumnHeaderStyle.None && columns[i].Hovered)
                            BaseControls.TreeLView.ExtendedListViews.uxTheme.Wrapper.DrawBackground("HEADER", "HEADERITEM", "HOT", hdc,
								lp_scr+last, tp, 
								colwid, headerBuffer,  
								lp, tp, r.Width-6, headerBuffer);
						else
                            BaseControls.TreeLView.ExtendedListViews.uxTheme.Wrapper.DrawBackground("HEADER", "HEADERITEM", "NORMAL", hdc,
								lp_scr+last, tp, 
								colwid, headerBuffer,  
								lp, tp, r.Width-6, headerBuffer);
						last += colwid;
					...
					{
                        BaseControls.TreeLView.ExtendedListViews.uxTheme.Wrapper.DrawBackground("HEADER", "HEADERITEM", "NORMAL", hdc, lp_scr + last, tp, r.Width - last - 2 + hscrollBar.Value, headerBuffer, r.Left, r.Top, r.Width, headerBuffer);
					}
				}
				catch
				{
