...
{
    /// <summary>
    /// 组件MyPictureBox，继承于PictureBox，图像盒
    /// </summary>
    public partial class MyPictureBox : System.Windows.Forms.PictureBox
    {
        ...
        public int imageMode = 6;   //图像模式 0：鼠标滚轮转动；1：鼠标点击控制图像大小；2：最佳尺寸；3：原始尺寸；4：鼠标移动查看图像；5：鼠标移动控制大小；6：默认情况。
        ...
        protected override void OnPaint(PaintEventArgs pe)
        {
            if (this.Image != null)
            {
                ...
                if (!this.isCtlMoving)  //更加图像模式，设置图片
                {
                    switch (imageMode)
                    {
                        case 0://鼠标滚轮转动
