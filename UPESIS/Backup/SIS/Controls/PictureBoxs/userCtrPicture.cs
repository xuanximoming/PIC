...
{
    /// <summary>
    /// 组件userCtrPicture，继承于PictureBox
    /// </summary>
    public partial class userCtrPicture : PictureBox
    {
        ...
        public void LoadFile(string file)
        {
            ...
            try
            {
                ...
                {
                    new SIS.FileOperator().WriteLog(string.Format("加载图片出错！{0}，文件大小：{1}\r\n", file, fs.Length));
                    fs.Close();
                    //System.IO.File.Delete(file);
                }
            }
            catch(Exception ex){
                new SIS.FileOperator().WriteLog(string.Format("加载图片出错！{0}，错误信息：{1}\r\n", file,ex.Message.ToString()));
            }
            if (fs != null)
                fs.Close();
        }
