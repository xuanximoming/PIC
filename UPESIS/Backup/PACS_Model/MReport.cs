...
{
    /// <summary>
    /// 实体类REPORT，合并REPORT和REPORT_SUB，检查报告表
    /// </summary>
    [Serializable]
    public class MReport : ILL.IModel
    {
        ...
        { }
        #region Model
        private string _EXAM_NO;                //检查号，该字段与EXAM_ACCESSION_NUM，即检查申请号一致，不同于Study里面的
        private string _exam_para;              //检查参数
        private string _description;            //检查所见
        private string _impression;             //诊断意见
        private string _recommendation;         //建议
        private string _dictator;               //口述人
        private string _transcriber;            //记录人
        ...
        private DateTime? _affirm_date_time;    //报告提交时间
        private int? _is_abnormal;              //阴阳性
        private int? _report_type;              //报告类型 0：初步报告；1：提交待审核;2:审核报告;3:冻结报告
        private string _print_template;         //打印模板 
        private int? _print_count;              //打印标志 空值：未打印，1：已打印
        private byte[] _report_name;            //报告名称
        private string _field_inf;              //域信息
